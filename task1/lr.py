...
optimizer1 = nn.SGD(nodes, grads)
# optimizer1 = nn.Adam(nodes, grads, weight_decay=0.001)
num_epochs_1 = 500
batch_size_1 = 64
loader_1 = data_loader(train_data1_x, train_data1_y, batch_size_1, True)
loss = nn.MSELoss()
...
num_epochs_2 = 500
batch_size_2 = 64
loader_2 = data_loader(train_data2_x, train_data2_y, batch_size_2, True)


def train(model: LogisticRegression, num_epochs, loader, loss_func, optimizer, train_x, train_y, test_x, test_y, loss_list, train_acc_list, test_acc_list):
    """
    Parameters
    --
    model
...
def visualize(train_acc_list, test_acc_list, loss_list):
    ...
    plt.savefig("./img/data1_lr_nopreprocess_sgd.png")
    plt.show()


# train(model1, num_epochs_1, loader_1, loss, optimizer1, train_data1_x, train_data1_y, test_data1_x, test_data1_y, loss_list_1, train_acc_1, test_acc_1)
# visualize(train_acc_1, test_acc_1, loss_list_1)

# train(model2, num_epochs_2, loader_2, loss, optimizer2, train_data2_x, train_data2_y, test_data2_x, test_data2_y, loss_list_2, train_acc_2, test_acc_2)
